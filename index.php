	...
	*/
	class http {
		...
		private static function translate($segments, $routes)
		{
			...
			$req_uri = urldecode(trim(trim($_SERVER['REQUEST_URI']), '/'));
	...
	abstract class controller extends http {
		...
		public function __get($libreria)
		{
			...
			if (!in_array($libreria, array_keys($libs))) {
				...
				#if (file_exists(SYSPATH."/$libreria.php")) {
					// Es una librería de sistema
					$namespace = 'enchinga';
					#} else {
					// Es una librería de app
					#$namespace = 'librerias';
					#}
				// La librería que pido con __get no viene namespaceada
				$clase = "\\$namespace\\$libreria";
				// Instanciamos la librería namespaceada, que se debe de cargar
				// según los
				$c = new $clase;
				$libs[$libreria] = &$c;
			}
		...
		/**
		 * Maquetar una vista con variables extraídas al scope local
		 *
		 * @param	string	$view	el nombre de la vista (Desde /)
		 * @param	string	$data	el arreglo de variables nombradas a extraer
		 * @return	void
		 * @author	Roberto Hidalgo
		 */
...
namespace enchinga\db\Mongo {
	...
	class Driver extends \MongoCollection
	{
		...
		public function set()
		{
			...
			if (count($args)==1) {
				if (is_array($args[0]) || is_object($args[0])) {
					$this->set = $this->set + (array) $args[0];
				} else {
		...
		public function update($where, $options=array())
		{
			...
			try {
				$result = parent::update((object) $where, $set, $opts);
				if ($safe && !$result['updatedExisting']) {
					return FALSE;
				}
			} catch (\MongoCursorException $e) {
				self::$last_error = $e->getMessage();
		...
		private function parseConditions($where)
		{
			...
			foreach ($where as $sujeto=>$predicado) {
				...
				if (is_array($predicado) && !preg_grep('/^\$/', array_keys($predicado))) {
